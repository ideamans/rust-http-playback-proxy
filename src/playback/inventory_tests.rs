...
mod tests {
    ...
    use crate::types::{DeviceType, Inventory, Resource};
    use std::sync::Arc;
    ...
    async fn test_inventory_with_complex_resource() {
        ...
        let mut resource = Resource::new(
            "PUT".to_string(),
            "https://api.example.com/data?id=123".to_string(),
        );
        ...
        headers.insert(
            ...
            crate::types::HeaderValue::Single("application/json".to_string()),
        );
        headers.insert(
            "x-rate-limit".to_string(),
            crate::types::HeaderValue::Single("100".to_string()),
        );
        ...
        assert_eq!(
            loaded_resource.error_message,
            Some("Rate limited".to_string())
        );
        assert!(loaded_resource.raw_headers.is_some());
        assert_eq!(
            loaded_resource.content_encoding,
            Some(crate::types::ContentEncodingType::Gzip)
        );
