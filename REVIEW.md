...
### 残存する高優先度問題
以下の Phase 4 の問題が残っています：
- 最終チャンクのタイミング制御（target_close_time の誤用）
...
- 🐛 **問題点**
  - トランザクション照合が「メソッド + パス + クエリ」だけでホストを見ていない (`src/playback/proxy.rs:85-118`)。MITM で複数オリジンを録画するユースケースでは `/static/app.js` などが衝突し、先に見つかった別ホストのレスポンスを返してしまう。少なくとも Host（できれば scheme/port も）をキーに含める必要がある
  - ストリーミングで最後のチャンクだけ `target_time` を無視し、`target_close_time` まで送信を遅延させている (`src/playback/proxy.rs:248-261`)。`target_close_time` は全転送時間そのものなので、1 チャンクのレスポンスでは TTFB 後ずっと無送信→終了間際に全バイト送出、という異常な挙動になる。複数チャンクでも最終チャンク前に不自然な空白が挟まる。最後のチャンクも他チャンク同様に `target_time` で送り、必要なら送信完了後に接続クローズを調整すべき

### 優先度別アクションアイテム

#### 高優先度（すぐに対応すべき）
1. 最終チャンクの送信を `target_time` ベースに戻し、`target_close_time` は接続クローズ時の待機にのみ使うよう修正する
2. ✅ **修正完了** トランザクション照合キーに Host（＋必要なら scheme/port）を含め、クロスオリジンで誤ったレスポンスを返さないようにする
   - ホスト情報を含めた照合ロジックに変更、後方互換性も維持
