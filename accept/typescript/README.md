...
This directory contains acceptance tests for the TypeScript/Node.js wrapper of http-playback-proxy.

## Purpose

These tests verify that the TypeScript wrapper works correctly in a production-like environment:
...
2. Copy the binary to the appropriate platform directory:
   ...
   cp target/release/http-playback-proxy typescript/bin/<platform>/
   ```
...
- Make sure you've built the Rust binary and copied it to `typescript/bin/<platform>/`
- Make sure you've built the TypeScript wrapper (`npm run build` in `typescript/`)
...
**Module not found:**
- Make sure you've run `npm install` in both `typescript/` and `accept/typescript/`
- The test uses a local file dependency: `"http-playback-proxy": "file:../../typescript"`

**Tests timeout:**
- The tests have a 10-second timeout per request
- If tests are slow, check your system resources
